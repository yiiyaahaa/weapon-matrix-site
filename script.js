...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        console.log('✅ DataHandler 已加载');
        
        // 确保 dataHandler 已存在（我们已经在 data-handler.js 中创建了）
        ...
        // 加载数据
        console.log('📥 开始加载数据...');
        const data = await window.dataHandler.loadAllData();
        
        console.log(`✅ 数据加载完成: ${data.weapons.length} 件武器, ${data.locations.length} 个地点`);
        ...
        // 更新界面显示
        updateWeaponCount(data.weapons.length);
        updateLocationCount(data.locations.length);
        
        // 初始化武器列表
        initWeaponList(data.weapons);
...
function createWeaponItem(weapon) {
    ...
    div.innerHTML = `
        <div class="weapon-info">
            ...
            <span class="weapon-star">${weapon.star}</span>
        </div>
        <div class="weapon-attributes">
            <span class="weapon-type">${weapon.type}</span>
            <span class="weapon-base">${weapon.baseAttr}</span>
            <span class="weapon-extra">${weapon.extraAttr}</span>
            <span class="weapon-skill">${weapon.skillAttr}</span>
        </div>
...
function initTabs() {
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            // 如果是属性搜索标签，初始化属性选择器
            if (tabId === 'attribute-search') {
                setTimeout(initAttributeSearch, 100);
            }
        });
    });
...
function createWeaponItem(weapon) {
    ...
    div.innerHTML = `
        <div class="weapon-select">
            <input type="checkbox" id="weapon-${weapon.id}" ${isSelected ? 'checked' : ''}
                   data-weapon="${weapon.name}">
            <label for="weapon-${weapon.id}" class="weapon-checkbox"></label>
        </div>
        <div class="weapon-info">
            ...
            <div class="weapon-attributes">
                ...
                <span class="attr-value attr-base">${weapon.baseAttr}</span>
                <span class="attr-type">附加:</span>
                <span class="attr-value attr-extra">${weapon.extraAttr}</span>
                <span class="attr-type">技能:</span>
                <span class="attr-value attr-skill">${weapon.skillAttr}</span>
            </div>
        </div>
        <div class="weapon-actions">
    ...
    // 绑定事件
    const checkbox = div.querySelector(`#weapon-${weapon.id}`);
...
function updateWeaponItemState(weaponName) {
    ...
    if (isSelected) {
        ...
        weaponItem.querySelector('input[type="checkbox"]').checked = true;
    } else {
        weaponItem.classList.remove('selected');
        weaponItem.querySelector('input[type="checkbox"]').checked = false;
    }
...
function initAttributeSearch() {
    if (!dataHandler || !dataHandler.isDataLoaded()) return;
...
function bindUtilityEvents() {
    ...
    // B站链接
    const bilibiliLink = document.querySelector('a[href*="bilibili"]');
    if (bilibiliLink) {
            ...
            <div class="search-result-item ${isSelected ? 'selected' : ''} ${isLocked ? 'locked' : ''}" 
                ...
                <div class="result-weapon-attrs">
                    <span class="attr-pair">
                        ...
                        <span class="attr-value">${weapon.baseAttr}</span>
                    </span>
                    <span class="attr-pair">
                        <span class="attr-label">附加:</span>
                        <span class="attr-value">${weapon.extraAttr}</span>
                    </span>
                    <span class="attr-pair">
                        <span class="attr-label">技能:</span>
                        <span class="attr-value">${weapon.skillAttr}</span>
                    </span>
                </div>
...
function clearAttributeSearch() {
    // 清除选择器
    document.getElementById('base-attr-select').value = '';
    document.getElementById('extra-attr-select').value = '';
    document.getElementById('skill-attr-select').value = '';
...
function createRecommendationCard(recommendation, selectedWeapons, lockedWeapons, isBest = false, rank = 1) {
    const engraving = recommender.recommendEngravingAttributes(
        selectedWeapons,
        lockedWeapons,
    ...
    return `
        <div class="recommendation-card ${isBest ? 'best-card' : ''}">
            <div class="card-header">
                ...
                <h5 class="card-title">
                    <i class="fas fa-map-marker-alt"></i> ${recommendation.location}
                </h5>
                <div class="card-scores">
                    <span class="score-badge">
                        <i class="fas fa-star"></i> ${recommendation.score}分
                    </span>
                    <span class="coverage-badge">
                        <i class="fas fa-percentage"></i> ${Math.round(recommendation.coverage * 100)}%
                    </span>
                </div>
            </div>
            ...
            <div class="card-body">
                <div class="engraving-recommendation">
                    ...
                    <div class="extra-skill-section">
                        <div class="section-title">${engraving.attributeType} (选1条)</div>
                ...
                <div class="matched-properties">
                    ...
                    <div class="match-stats">
                        <div class="match-stat">
                            ...
                            <div class="stat-bar">
                                <div class="stat-fill" style="width: ${(recommendation.matched.base / selectedWeapons.length) * 100}%"></div>
                            </div>
                            <span class="stat-value">${recommendation.matched.base}/${selectedWeapons.length}</span>
                        </div>
                        <div class="match-stat">
                            <span class="stat-label">附加属性</span>
                            <div class="stat-bar">
                                <div class="stat-fill" style="width: ${(recommendation.matched.extra / selectedWeapons.length) * 100}%"></div>
                            </div>
                            <span class="stat-value">${recommendation.matched.extra}/${selectedWeapons.length}</span>
                        </div>
                        <div class="match-stat">
                            <span class="stat-label">技能属性</span>
                            <div class="stat-bar">
                                <div class="stat-fill" style="width: ${(recommendation.matched.skill / selectedWeapons.length) * 100}%"></div>
                            </div>
                            <span class="stat-value">${recommendation.matched.skill}/${selectedWeapons.length}</span>
                        </div>
                    </div>
                </div>
            </div>
...
function createAnalysisReport(selectedWeapons, lockedWeapons) {
    const analysis = recommender.analyzeWeaponsAttributes(selectedWeapons);
...
function getWeaponAttributesText(weaponName) {
    ...
    return `${weapon.baseAttr} / ${weapon.extraAttr} / ${weapon.skillAttr}`;
}
...
function exportResults() {
    try {
        ...
        exportContent += textNodes.filter(text => text).join('\n');
